...
func publishMessage(t *testing.T, ch *amqp.Channel, exchange, key, body string, mandatory bool, pub amqp.Publishing) {
	t.Helper()
	pub.Body = []byte(body)
...
// consumeMessage starts a consumer and returns the delivery channel and the actual consumer tag.
// If consumerTag is empty, one will be generated by the client library.
func consumeMessage(t *testing.T, ch *amqp.Channel, queueName string, consumerTag string, autoAck bool) (deliveries <-chan amqp.Delivery, actualConsumerTag string) {
	t.Helper()
...
// expectChannelClose sets up NotifyClose and asserts the expected error.
func expectChannelClose(t *testing.T, ch *amqp.Channel, expectedCode int, expectedReasonContains string) {
	t.Helper()
...
// expectNoChannelClose ensures no channel close notification is received within a duration.
func expectNoChannelClose(t *testing.T, ch *amqp.Channel, duration time.Duration) {
	t.Helper()
	closedCh := make(chan *amqp.Error, 1)
	ch.NotifyClose(closedCh)
...
// expectMessage waits for a message and asserts its body and optionally redelivered status.
// Returns the received message.
func expectMessage(t *testing.T, deliveries <-chan amqp.Delivery, expectedBody string, expectedRedelivered *bool, timeout time.Duration) amqp.Delivery {
	t.Helper()
	select {
...
// expectNoMessage ensures no message is received within a duration.
func expectNoMessage(t *testing.T, deliveries <-chan amqp.Delivery, duration time.Duration) {
	t.Helper()
	select {
	case msg := <-deliveries:
...
// setupQueueAndConsumer is a common setup for ack/nack/reject tests.
func setupQueueAndConsumer(t *testing.T, ch *amqp.Channel, queueName string, autoAck bool) (q amqp.Queue, deliveries <-chan amqp.Delivery, consumerTag string) {
	t.Helper()
	var err error
	...
	deliveries, actualConsumerTag := consumeMessage(t, ch, q.Name, ctag, autoAck)
