...
func (s *server) routeThroughExchange(vhost *vHost, ex *exchange, routingKey string, visited, seenQueues map[string]bool, queues *[]string) error {
	if visited[ex.Name] {
		return nil
	}
	...
	for _, destinationName := range matchedExchanges {
		...
		if destination == nil {
			s.Warn("Exchange '%s' is bound to missing exchange '%s'; skipping", ex.Name, destinationName)
			continue
		}
		if err := s.routeThroughExchange(vhost, destination, routingKey, visited, seenQueues, queues); err != nil {
			return err
		}
	}
	...
	if alternateEx == nil {
		s.Warn("Alternate exchange '%s' of exchange '%s' does not exist; message stays unroutable", alternate, ex.Name)
		return nil
	}
	s.Debug("Forwarding message from exchange '%s' to alternate exchange '%s'", ex.Name, alternate)
	return s.routeThroughExchange(vhost, alternateEx, routingKey, visited, seenQueues, queues)
}
